...
float duration() {
  ...
  digitalWrite(trigPin, LOW);
  unsigned int duration = pulseIn(echoPin, HIGH, 30000);
  return duration;
}

float distance(float dur) {
  int temp = 20; // Adjust as needed
  float speedOfSound = soundSpeed[temp - 10];
  float mm = ((speedOfSound) * dur) / 2;
  return mm;
}
